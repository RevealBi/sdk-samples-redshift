...
const authenticationProvider = async (userContext, dataSource) => {
    if (dataSource instanceof reveal.RVRedshiftDataSource) {
      const username = userContext.properties.get("Username") || process.env.REDSHIFT_USERNAME;
      const password = userContext.properties.get("Password") || process.env.REDSHIFT_PASSWORD;
...
const dataSourceProvider = async (userContext, dataSource) => {
    if (dataSource instanceof reveal.RVRedshiftDataSource) {
        const host = userContext.properties.get("Host") || process.env.REDSHIFT_HOST;
        const database = userContext.properties.get("Database") || process.env.REDSHIFT_DATABASE;
        const schema = userContext.properties.get("Schema") || process.env.REDSHIFT_SCHEMA;        
    ...
    try {
        ...
        switch (dataSourceItem.id) {
            ...
            case "TenMostExpensiveProducts":
                dataSourceItem.customQuery = "Select * from ten_most_expensive_products";
                break;

            // Example of how to use a view with parameter in Redshift
            case "CustOrderHist":
                dataSourceItem.customQuery = "select * from cust_order_hist";
                break;

            // Example of how to use a standard SQL statement with parameter
            case "CustOrdersOrders":
                dataSourceItem.customQuery = `SELECT OrderID, OrderDate, RequiredDate, ShippedDate FROM Orders WHERE CustomerID = '${customerId}' ORDER BY OrderID`;
                break;
...
const dataSourceItemFilter = async (userContext, item) => {
    if (!(item instanceof reveal.RVRedshiftDataSourceItem)) {
        return false;
    }
